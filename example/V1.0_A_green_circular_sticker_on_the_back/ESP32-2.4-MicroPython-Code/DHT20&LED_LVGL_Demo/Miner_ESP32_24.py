...
touch = xpt2046(cs=33, spihost=VSPI_HOST, mosi=-1, miso=-1, clk=-1, cal_x0=300,cal_x1=3900,cal_y0 = 423, cal_y1=3948,transpose = 0)
...
class TEM_HUM():
    def __init__(self, ui_Screen1):
        # Read temperature and humidity values from DHT20 sensor
        global Tem, Hum
        Tem = sensor.dht20_temperature()
        Hum = sensor.dht20_humidity()
        
        # Update temperature and humidity display on UI interface
        ui_Label4.set_text(f"{round(Tem)} °C")  # Update temperature display
        ui_Label5.set_text(f"{round(Hum)} %")       # Update humidity display

# Ensure the TEM_HUM class is instantiated in the program and ui_Screen1 is passed as a parameter
def update_temperature_humidity():
    global Tem, Hum
    # Read temperature and humidity values from DHT20 sensor
    sensor.read_dht20()  # Ensure read_dht20() is called to update sensor data
    Tem = sensor.dht20_temperature()
    Hum = sensor.dht20_humidity()
...
def main_loop():
    while True:
        ...
        time.sleep(1)  # Pause for 1 second, then update again
